...
public class TvGridLayoutManagerDanLie extends GridLayoutManager {
    ...
    public TvGridLayoutManagerDanLie(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr, defStyleRes);
    }

    public TvGridLayoutManagerDanLie(Context context, int spanCount) {
        super(context, spanCount);
    }

    public TvGridLayoutManagerDanLie(Context context, int spanCount, int rowCount, int orientation, boolean reverseLayout, TvGridView tvGridView) {
...
//        ★★★
            int fromPos = mTvGridView.getCurrentFocusedChildPosition();
//            Log.e("cccqqq111",fromPos+"");
...
//       ****************** 边界焦点处理逻辑******************
        if (fromPos >= count) {
            //如果下一个位置超出item的总数，则返回当前的View，即焦点不动
            return focused;
        } else if (fromPos<0){
            return upFocusView;
        }
